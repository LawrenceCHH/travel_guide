...
7 類 token，並掃描待版控檔案是否含這些字串，命中即報錯：
  1. 訂房確認號 / 訂單編號（如 Booking Reference、確認碼）
  2. 訂房人姓名（Name / 姓名 / 訂房人 標籤後的值）
  3. 航班號（2 碼航空公司代碼 + 2~4 碼數字，如 BR225、CI702）
  4. 電話號碼（含國際冠碼、市話、手機各種格式）
  5. Email 地址
  6. 護照號碼（Passport No. 標籤後的值）
  7. 門牌地址（Address / 地址 標籤後的值）

飯店/住宿全名本身**不在**掃描範圍內——單獨的飯店名不具識別性（公開可查、不指向特定人）。
真正的風險在於「飯店名/門牌地址 + 真實姓名 + 入住日期」疊在一起會揭露真人在哪個時間點
出現在哪個確切地點；姓名與門牌地址已各自獨立列為必刪類別（1、7），足以打斷這個疊加，
不需要額外把飯店名也列為違規 token。
