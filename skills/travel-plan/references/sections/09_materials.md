...
- **飲食限制只在明講時才套用**：`trip_context.party.diet_notes` 非空時，該限制是硬篩選條件——不符合的候選不得列入 `food.picks`，符合的項目要在 `dietary_tags` 標出對應限制。`diet_notes` 為 `null`（使用者沒提過）時，**不做任何飲食臆測**（不預設「不辣」「不吃內臟」等），美食選材單純依 §9.2 的熱門度/評論數/跨分類取樣邏輯進行，`dietary_tags` 留空即可。不得因為想「保險一點」就自行加上使用者沒要求的飲食限制，那會不必要地縮小選材範圍。
